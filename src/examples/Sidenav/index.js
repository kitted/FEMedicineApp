...
function Sidenav({ color, routes, subTitle, ...rest }) {
  ...
  const renderRoutes = routes.map(
    ({ type, name, icon, title, collapse, noCollapse, key, href, route, permission }) => {
      let returnValue;
      let isPermission = Object?.entries(permission)?.map(([key, value]) => ({
        value,
      }));
      if (type === "collapse" && isPermission.some((value) => value.value === user.role)) {
        if (href) {
          returnValue = (
            <Link href={href} key={key} target="#" rel="noreferrer" sx={{ textDecoration: "none" }}>
              <SidenavCollapse
                name={name}
                icon={icon}
        ...
        returnValue = (
          <Link href={route} key={key} target="" rel="noreferrer" sx={{ textDecoration: "none" }}>
            <SidenavCollapse
              name={name}
              icon={icon}
              active={key === collapseName}
      ...
      <SoftBox pt={3} pb={1} px={4} textAlign="center">
        ...
        <SoftBox component={NavLink} to="/" display="flex" alignItems="center">
          ...
          >
            ...
            <SoftTypography component="h6" variant="button" fontWeight="bold">
              <span className="text-center ml-[28px]">{subTitle}</span>
            </SoftTypography>
            </div>
          </SoftBox> */}
        </SoftBox>
